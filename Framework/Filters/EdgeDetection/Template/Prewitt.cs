...
namespace Framework.Filters.EdgeDetection.Template
{
    ...
    /// 
    /// </summary>
    [Filter("Prewitt", "Prewitt Edge Detection", "***",
            new string[] { "Edge Detection", "Template Based" }, true)]
    [EdgeDetectionMetric()]
    public class Prewitt : SpatialDomainFilter
    {
        ...
        public override byte[,] ApplyFilter(byte[,] iImageSrc, SortedDictionary<string, object> iConfigs)
        {
            ...
            byte[,] tmp_ret = Kernel2DBatch.Evaluate(
                    iImageSrc,
                    new Kernel2D[] {
                            new Kernel2D(0, (1/3.0), new double[,] { { -1, -1, -1 }, { 0, 0, 0 }, { 1, 1, 1 } }), 
                            new Kernel2D(0, (1/3.0), new double[,] { { -1, 0, 1 }, { -1, 0, 1 }, { -1, 0, 1 } })
                        },
