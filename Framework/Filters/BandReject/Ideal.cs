...
namespace Framework.Filters.BandReject
{

    /// <summary>
    /// Apply the ideal band reject to an image.
    /// </summary>
    /// <remarks>
    /// The ideal band reject filter cuts off all frequencies in a range of 
    /// distances from the center.
    /// (Gonzalez, R. C., Woods, R. E.. Digital Image Processing.2.Prentice Hall)
    /// </remarks>
    [Filter("Ideal", "Ideal Band Reject Filter",
            "The ideal band reject filter cuts off all frequencies in a range of distances from the center.",
            new string[] { "Band Reject" }, true)]
    [SmoothMetric]
    public class Ideal : FrequencyDomainFilter<FourierTransform>
    {
        ...
        public override ComplexImage ApplyFilter(ComplexImage complexImg, SortedDictionary<string, object> configs)
        {
            ...
            double d0_lower = d0 - w / 2.0;
            double d0_upper = d0 + w / 2.0;
            double d;
