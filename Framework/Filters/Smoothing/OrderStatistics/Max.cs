...
{
    /// <summary>
    /// Apply the max filter to an image.
    /// </summary>
    /// <remarks>
    /// Computes the output pixels by taking the maximum intensity
    /// of the neighborhood of the pixel.
    /// </remarks>
    [Filter("Max", "Max Filter",
            "Computes the output pixels by taking the maximum intensity" +
            " of the neighborhood of the pixel.",
            new string[] { "Smoothing", "Order-Statistics" }, true)]
    [SmoothMetric]
    public class Max : SpatialDomainFilter
    {
            ...
            byte[,] tmp_ret = Kernel2D.ApplyFunction(
                    ...
                    delegate(Tuple<Point, Point>[] mappings, byte[,] op)
                    {
                        ...
                        for (i = 0; i < mappings.Length; ++i)
                        {
                            ...
                            {
                                ret = Math.Max(ret, op[mappings[i].Item2.X, mappings[i].Item2.Y]);
                            }
                        }
